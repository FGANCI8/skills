...
|---|---|---|---|---|---|---|
| [OpenAI Agents SDK Python](https://pypi.org/project/openai-agents/) | 0.18.2, 2026-07-11 | agents, tools, handoffs, guardrails, sessions, tracing, HITL | implementação futura separada | qualquer runtime V1.1 de referência | médio | não é dependência/extra da V1.1; provider real hard-forbidden |
...
8. Esta camada Agent OS V1.1/PR B não contém runtime nem adapter OpenAI; qualquer integração OpenAI pertence a uma camada futura e exige autorização própria, implementação, approval verifier e revisão separados, enquanto provider real permanece proibido nesta fase. LangGraph apenas para durabilidade comprovada.
9. Nenhuma dependência de provider ou chamada real é permitida para validar a referência mock.
10. Approval evidence, ownership, limites cumulativos e métricas usam contratos fechados; texto/handoff nunca amplia autoridade.
